...
from HGCNMNoEmbed.mambaForEEG import Mamba, MambaConfig, RMSNorm
...
def from_pretrained(name: str):
    ...
    try:
        ...
        print(
            "The from_pretrained function pulls weights from HuggingFace and thus needs transformers to be installed (pip install transformers)")
        return
    ...
    config = MambaLMConfig(d_model=config_data['d_model'], n_layers=config_data['n_layer'],
...
class MambaLM(nn.Module):
    def __init__(self, lm_config: MambaLMConfig, args):
        ...
        self.mlp = nn.Linear(self.lm_config.vocab_size * 3, self.nclass, bias=False)
    ...
    def generate(self, tokenizer, prompt: str, num_tokens: int = 50, batch_size: int = 1, sample: bool = True,
